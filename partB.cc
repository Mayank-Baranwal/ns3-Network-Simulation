...
NS_LOG_COMPONENT_DEFINE ("App_Part_B");
...
App_Part_B::App_Part_B(): m_Socket(0), m_Peer(), m_PacketSize(0), m_NPackets(0), m_DataRate(0), m_SendEvent(), m_Running(false), m_PacketsSent(0)
{}
...
void App_Part_B::ScheduleTx()
{
	if (m_Running)
	{
		...
		m_SendEvent = Simulator::Schedule(tNext, &App_Part_B::SendPacket, this);
	}
}
...
int main()
{
    ...
    YeahPacketLossStats = fopen ("PartB_Yeah_congestion_loss.txt","w");
    YeahCwndStats = fopen ("PartB_Yeah_cwnd.csv","w");
    YeahThroughputStats = fopen ("PartB_Yeah_tp.csv","w");
    YeahGoodputStats = fopen ("PartB_Yeah_gp.csv","w");

    // TCP Yeah Stimulation

	cout << "Stage 4a: Initiating TCP Yeah Simulation" << endl;
	Ptr<Socket> YeahSocket = YeahFlow(InetSocketAddress( i_h4r2.GetAddress(0), port), port, nodes.Get(0), nodes.Get(5), startTime, startTime+runningTime+30, packetSize, totalPackets, transferSpeed, startTime, startTime+runningTime+30);
	// Measure PacketSinks
    ...
    HyblaPacketLossStats = fopen ("PartB_Hybla_congestion_loss.txt","w");
    HyblaCwndStats = fopen ("PartB_Hybla_cwnd.csv","w");
    HyblaThroughputStats = fopen ("PartB_Hybla_tp.csv","w");
    HyblaGoodputStats = fopen ("PartB_Hybla_gp.csv","w");

    // TCP Hybla Simulation

	cout << "Stage 4b: Initiating TCP Hybla Simulation" << endl;
	Ptr<Socket> HyblaSocket = HyblaFlow(InetSocketAddress(i_h5r2.GetAddress(0), port), port, nodes.Get(1), nodes.Get(6), startTime+30, startTime+runningTime+30, packetSize, totalPackets, transferSpeed, startTime, startTime+runningTime+30);
	// Measure PacketSinks
    ...
    WestwoodPacketLossStats = fopen ("PartB_Westwood_congestion_loss.txt","w");
    WestwoodCwndStats = fopen ("PartB_Westwood_cwnd.csv","w");
    WestwoodThroughputStats = fopen ("PartB_Westwood_tp.csv","w");
    WestwoodGoodputStats = fopen ("PartB_Westwood_gp.csv","w");
	Ptr<Socket> westwoodPSocket = WestwoodFlow(InetSocketAddress(i_h6r2.GetAddress(0), port), port, nodes.Get(2), nodes.Get(7), startTime+30, startTime+runningTime+30, packetSize, totalPackets, transferSpeed, startTime, startTime+runningTime+30);
	// Measure PacketSinks
